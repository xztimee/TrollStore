...
{
	if ([[NSFileManager defaultManager] fileExistsAtPath:LSJBRootPath(@"/Library/MobileSubstrate/DynamicLibraries/AppSyncUnified-FrontBoard.dylib")]) {
		return YES;
	}
	return NO;
}
...
{
	return luiseStoreInstalledAppContainerPathsInternal(TS_ACTIVE_MARKER);
}
...
{
	return luiseStoreInstalledAppBundlePathsInternal(TS_ACTIVE_MARKER);
}

NSArray *luiseStoreInactiveInstalledAppBundlePaths(void)
{
	return luiseStoreInstalledAppBundlePathsInternal(TS_INACTIVE_MARKER);
}
