# Credits

## Original TrollStore

LuiseStore is a fork of [TrollStore](https://github.com/opa334/TrollStore) by opa334.

All credits for the original work and the CoreTrust bug exploitation go to:

### Main Developer
- **opa334** (Lars Fröder) - Original TrollStore developer
  - Email: opa334@protonmail.com
  - GitHub: https://github.com/opa334/TrollStore

### Contributors to Original TrollStore

- **[@alfiecg_dev](https://twitter.com/alfiecg_dev/)** - Found the CoreTrust bug that allows TrollStore to work through patchdiffing and worked on automating the bypass.

- **Google Threat Analysis Group** - Found the CoreTrust bug as part of an in-the-wild spyware chain and reported it to Apple.

- **[@LinusHenze](https://twitter.com/LinusHenze)** - Found the installd bypass used to install TrollStore on iOS 14-15.6.1 via TrollHelperOTA, as well as the original CoreTrust bug used in TrollStore 1.0.

### Additional Credits

- **CoolStar** - Original uicache implementation
- **Procursus Team** - Modified uicache work
- All other contributors to the original TrollStore project

## Resources

- [Fugu15 Presentation](https://youtu.be/rPTifU1lG7Q)
- [Write-Up on the first CoreTrust bug](https://worthdoingbadly.com/coretrust/)
- [Original TrollStore Repository](https://github.com/opa334/TrollStore)

## varClean

The integrated filesystem cleanup feature is based on
[varClean](https://github.com/m1337v/varClean), a TrollStore port of RootHide
Manager's cleanup function. Copyright © 2023 RootHide, used under the MIT
License.

## LuiseStore Modifications

LuiseStore maintains the same functionality as TrollStore while being rebranded for specific use cases.

For the complete license information, see the [LICENSE](LICENSE) file.
